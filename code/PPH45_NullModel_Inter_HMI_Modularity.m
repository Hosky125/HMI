...
% 读取文件
% PP-Plants-Pollinations-174个网络
% PH-Plants-Herbivores-69个网络   

% 使用 dir 函数获取当前路径下某个文件夹中所有 .csv 文件的信息
PPH_fileInfo = dir(fullfile(pwd, 'DATA/PPH_45','*.txt'));

% 初始化一个 cell 数组来存储所有文件名
PPH_fileNames = {PPH_fileInfo.name}';

networksize = zeros(length(PPH_fileInfo)/2,4);%4列依次表示：ph的行数、ph的列数、pp的行数、pp的列数
linksnum = zeros(length(PPH_fileInfo)/2,2);%ph的连接数、pp的连接数
%connectance = zeros(69*174,1);

iterations = 1000;
iter=100;
Q_mean_max_total = zeros(iterations, length(PPH_fileInfo)/2);
HMI_monolayer_total =  zeros(iterations, length(PPH_fileInfo)/2);
Q_multilayer_max_total = zeros(iterations, length(PPH_fileInfo)/2);
HMI_multilayer_total =  zeros(iterations, length(PPH_fileInfo)/2);
...
for pph_index=1:45
    ...
    count
    % 构建完整的文件路径
    PPH_filePath1 = fullfile(pwd,'DATA/PPH_45',PPH_fileNames(pph_index));
    PPH_filePath2 = fullfile(pwd,'DATA/PPH_45',PPH_fileNames(pph_index+45));

    % 读取文件
    PH=importdata(PPH_filePath1{1});
    PH=PH.data;

    PP=importdata(PPH_filePath2{1});
    PP=PP.data;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %calculate network属性
    networksize(count,1) = size(p_h,1);
    networksize(count,2) = size(p_h,2);
    networksize(count,3) = size(p_p,1);
    networksize(count,4) = size(p_p,2);
    linksnum(count,1) = sum(sum(p_h));
    linksnum(count,2) = sum(sum(p_p));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    csvwrite('HMI_monolayer_total_PPH_Inter_NullModel.csv', HMI_monolayer_total);
    csvwrite('HMI_multilayer_total_PPH_Inter_NullModel.csv', HMI_multilayer_total);
    csvwrite('Q_mean_max_total_PPH_Inter_NullModel.csv', Q_mean_max_total);
    csvwrite('Q_multilayer_max_total_PPH_Inter_NullModel.csv', Q_multilayer_max_total);
    csvwrite('networksize_PPH.csv', networksize);
    csvwrite('linksnum_PPH.csv', linksnum);
