...
SpaNet_fileInfo = dir(fullfile(pwd, 'DATA/SpaNet_Intra_NullModel/'));
SpaNet_fileNames = {SpaNet_fileInfo.name}';
SpaNet_fileNames = SpaNet_fileNames(3:11);

for ID=1:9
    ID
    tic
    SpaNet_fileInfo = dir(fullfile(pwd, 'DATA/SpaNet_Intra_NullModel/',SpaNet_fileNames{ID},'/','*.txt'));
    Txt_fileNames = {SpaNet_fileInfo.name}';
    SpaNet_NullModel_Data=cell(1000,length(Txt_fileNames)/1000);

    for i=1:length(Txt_fileNames)
        SpaNet_filePath = fullfile(pwd,'DATA/SpaNet_Intra_NullModel/',SpaNet_fileNames{ID},'/',Txt_fileNames(i));
        m=mod(i,1000);
        if m==0
            m=1000;
        end
        ...
        Data=importdata(SpaNet_filePath{1});% 读取文件
        SpaNet_NullModel_Data{m,n}=Data.data;
    end
    save_filename = strcat("SpaNet",sprintf('%02d', ID),"_NullModel");
    save(save_filename,"SpaNet_NullModel_Data")
    toc
end
%-------------------------------------------------------------------------%
...
for ID=1:9
    ...
    csvwrite('HMI_monolayer_total_SpaNet_Intra_NullModel.csv', HMI_Random_total);
    csvwrite('HMI_multilayer_total_SpaNet_Intra_NullModel.csv', HMI_multilayer_Random_total);
    csvwrite('Q_mean_max_total_SpaNet_Intra_NullModel.csv', Q_Random_total);
    csvwrite('Q_multilayer_max_total_SpaNet_Intra_NullModel.csv', Q_multilayer_Random_total);
